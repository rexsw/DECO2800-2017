	...
	public void handleInput(long pauseTime) {
		...
		if(this.downKeys.contains(Input.Keys.N) && !this.cameraPosition.isEmpty()){
			...
			if(this.switcher == 0){
				...
				x *= -1;
				y *= -1;
				if(this.camera.position.x > nextPosition.get(0) || (this.camera.position.x <= nextPosition.get(0)&& this.camera.position.x >0)){
					this.camera.translate(x, 0);
				}
				if(this.camera.position.y > nextPosition.get(1) || 
						(this.camera.position.y <= nextPosition.get(1))){
		...
		if (cursorX > windowWidth - pxTolerance) { // moving right
			if (cursorY < pxTolerance) {
				// move up and right
				this.camera.translate((float) 0.7071 * speed * this.camera.zoom, (float) 0.7071 * speed * this.camera.zoom, 0);
			} else {
				// move right
				this.camera.translate(1 * speed * this.camera.zoom, 0, 0);
			}
	...
	public void setInputProcessor() {
		...
		inputMultiplexer.addProcessor(new InputAdapter() {
			...
			public boolean scrolled(int amount) {
				...
				if (InputProcessor.this.camera.zoom > 0.5 && amount == -1) { // zoom in
				...
				} else if (InputProcessor.this.camera.zoom < 10 && amount == 1) { // zoom out
					InputProcessor.this.camera.zoom *= 1.2;
				}
